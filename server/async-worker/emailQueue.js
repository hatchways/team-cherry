...
module.exports = async function emailQueue() {
  ...
  sendEmail.process(async (job) => {
    ...
    for (let company of companies) {
      ...
      //get all mentions
      companyMentions[company.name] = companyMentions[company.name].concat(
        mentions
      );

      //sort by popularity
      companyMentions[company.name] = companyMentions[company.name].sort(
        (a, b) => {
      ...
      companyMentions[company.name] = companyMentions[company.name].filter(
        (current) => {
      ...
      if (companyMentions[company.name].length > 10) {
        companyMentions[company.name] = companyMentions[company.name].slice(
          0,
          10
        );
      }
